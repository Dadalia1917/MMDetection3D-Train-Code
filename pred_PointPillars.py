...
def parse_args():
    parser = ArgumentParser(description='PointPillars inference demo')
    parser.add_argument(
        '--pcd', 
...
def get_checkpoint_file(dataset):
    """根据数据集选择检查点文件"""
    checkpoint_map = {
        'kitti-3class': 'runs/outputs/PointPillars_kitti-3class/latest.pth',
        'kitti-car': 'runs/outputs/PointPillars_kitti-car/latest.pth',
    }
...
def main():
    ...
    if not os.path.exists(checkpoint_file):
        raise FileNotFoundError(
            ...
            f"Please train the model first using: python train_PointPillars.py --dataset {args.dataset} --amp"
        )
    ...
    if not args.no_save_vis:
        ...
        if vis_files:
            ...
            result_filename = f"result_PointPillars_{args.dataset}.png"
