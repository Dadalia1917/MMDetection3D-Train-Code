...
def parse_args():
    parser = argparse.ArgumentParser(description='Train SMOKE mono3D detector')
    parser.add_argument(
        '--dataset', 
...
def ensure_smoke_models():
    """确保SMOKE所需的预训练模型已下载"""
    models_dir = Path('./models/smoke')
    ...
    # SMOKE KITTI预训练权重
    smoke_url = 'https://download.openmmlab.com/mmdetection3d/v0.1.0_models/smoke/smoke_dla34_pytorch_dlaneck_gn-all_8x4_6x_kitti-mono3d_20210929_015553-d46d9bb0.pth'
    smoke_path = models_dir / 'smoke_dla34_pytorch_dlaneck_gn-all_8x4_6x_kitti-mono3d_20210929_015553-d46d9bb0.pth'
    
    print_log("Checking SMOKE model dependencies...", logger='current')
    
    # 下载DLA34权重
    dla34_result = download_file(dla34_url, dla34_path)
    
    # 可选：下载SMOKE预训练权重（用于推理）
    smoke_result = download_file(smoke_url, smoke_path)
    
    return dla34_result, smoke_result
...
def main():
    ...
    # 确保预训练模型已下载
    dla34_path, smoke_path = ensure_smoke_models()
    ...
    # 启用AMP训练
    if args.amp:
        # 将OptimWrapper替换为AmpOptimWrapper以支持混合精度训练
        if hasattr(cfg, 'optim_wrapper') and cfg.optim_wrapper.get('type') == 'OptimWrapper':
            print_log('Enabling AMP training for SMOKE...', logger='current', level=logging.INFO)
            cfg.optim_wrapper.type = 'AmpOptimWrapper'
            # 确保保留原有的optimizer和clip_grad设置
            if 'loss_scale' not in cfg.optim_wrapper:
                cfg.optim_wrapper.loss_scale = 'dynamic'
        else:
            print_log('AMP training enabled', logger='current', level=logging.INFO)
