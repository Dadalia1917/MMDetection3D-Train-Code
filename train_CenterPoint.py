...
def ensure_centerpoint_models():
    """确保CenterPoint所需的预训练模型已下载"""
    models_dir = Path('./models/centerpoint')
    models_dir.mkdir(parents=True, exist_ok=True)
    
    # CenterPoint nuScenes预训练权重
    centerpoint_nus_url = 'https://download.openmmlab.com/mmdetection3d/v1.0.0_models/centerpoint/centerpoint_0075voxel_second_secfpn_dcn_4x8_cyclic_20e_nus/centerpoint_0075voxel_second_secfpn_dcn_4x8_cyclic_20e_nus_20220810_030004-9061688e.pth'
    centerpoint_nus_path = models_dir / 'centerpoint_0075voxel_second_secfpn_dcn_4x8_cyclic_20e_nus_20220810_030004-9061688e.pth'
    
    print_log("Checking CenterPoint model dependencies...", logger='current')
    
    # 可选：下载预训练权重（用于推理）
    centerpoint_nus_result = download_file(centerpoint_nus_url, centerpoint_nus_path)
    
    return centerpoint_nus_result


def get_config_file(dataset):
    """Get config file path based on dataset"""
    config_map = {
        'nuscenes': 'configs/centerpoint/centerpoint_voxel0075_second_secfpn_head-circlenms_8xb4-cyclic-20e_nus-3d.py',
        'waymo': 'configs/centerpoint/centerpoint_pillar02_second_secfpn_head-circlenms_4xb4-cyclic-12e_waymoD5-3d-3class.py'
    }
    return config_map.get(dataset, config_map['nuscenes'])
...
def main():
    ...
    centerpoint_nus_path = ensure_centerpoint_models()
    ...
    if args.work_dir is not None:
        ...
        if hasattr(args, 'dataset') and args.dataset:
            model_name = f'CenterPoint_{args.dataset}'
