...
def enhance_3d_visualization(vis_img_path, labels, scores, bboxes):
    ...
    # 保存增强后的图像
    result_path = "result_MonoFlex_3d.png"
            ...
            if results and 'predictions' in results and results['predictions']:
                ...
                if 'bboxes_3d' in predictions and len(predictions['bboxes_3d']) > 0:
                    ...
                    if enhance_3d_visualization(temp_vis_file.name, labels, scores, bboxes):
                        print_log(f"Enhanced 3D detection visualization saved to: result_MonoFlex_3d.png", logger='current')
                    else:
                        # 如果增强失败，直接复制原图
                        import shutil
                        shutil.copy2(temp_vis_file.name, "result_MonoFlex_3d.png")
                        print_log(f"3D detection visualization saved to: result_MonoFlex_3d.png", logger='current')
                else:
                    # 没有检测结果，直接复制
                    import shutil
                    shutil.copy2(temp_vis_file.name, "result_MonoFlex_3d.png")
